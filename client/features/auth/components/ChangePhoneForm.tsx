...
export function ChangePhoneForm({ variant = "settings" }: ChangePhoneFormProps) {
  ...
  const formContent = (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <div className={cn("space-y-6", !isProfile && "bg-white p-8 rounded-[24px] border border-divider shadow-sm")}>
          {!isProfile && (
          ...
          {isProfile && (
            <div className={cn("space-y-1", isRtl && "text-right")}>
              <p className="text-sm font-semibold text-primary">{t("phone_settings_title")}</p>
              <p className="text-sm text-slate-500">{t("phone_settings_desc")}</p>
            </div>
          )}
        ...
        <div
          className={cn(
            "flex w-full",
            isProfile ? "justify-end pt-2" : "mt-6",
            isRtl && !isProfile ? "justify-start" : "",
            !isRtl && !isProfile ? "justify-end" : "",
          )}
          dir="ltr"
        >
          {!codeSent ? (
            <Button
              ...
              className={cn(
                "h-12 px-8 min-w-[140px] rounded-xl font-bold",
                isProfile
                  ? "bg-primary hover:bg-primary/90 text-white"
                  : "bg-[#1B254B] hover:bg-[#1B254B]/90",
              )}
            >
              {isRequesting ? <Loader2 className="me-2 h-4 w-4 animate-spin" /> : null}
