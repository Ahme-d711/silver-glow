  ...
  return (
    <View className="items-center py-10">
      <View className="relative">
        ...
        <View className={`absolute bottom-1 ${currentLanguage === 'ar' ? 'left-1' : 'right-1'} bg-primary w-10 h-10 rounded-full items-center justify-center border-4 border-white`}>
          <Feather name="camera" size={16} color="white" />
        </View>
      </View>
      
      <Text className="text-slate-800 text-2xl font-bold mt-5">
        {user?.name || t('auth.user_fallback')}
      </Text>
      <View className="bg-emerald-50 px-4 py-1 rounded-full mt-2 flex-row items-center">
        <View className={`w-2 h-2 rounded-full bg-emerald-500 ${currentLanguage === 'ar' ? 'ml-2' : 'mr-2'}`} />
        <Text className="text-emerald-600 text-xs font-bold uppercase tracking-widest">
          {user?.role || t('auth.role_user')}
        </Text>
      </View>
    </View>
  );
