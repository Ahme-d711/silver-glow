...
export const AccountInfoRow: React.FC<AccountInfoRowProps> = ({ icon, label, value, isLast }) => {
  ...
  return (
    <View className={`flex-row items-center py-5 ${!isLast ? 'border-b border-slate-50' : ''}`}>
      <View className={`w-10 h-10 rounded-xl bg-slate-50 items-center justify-center ${currentLanguage === 'ar' ? 'ml-4' : 'mr-4'}`}>
      ...
      <View className="flex-1">
        ...
        <Text className={`text-slate-800 text-[16px] font-bold mt-0.5 ${currentLanguage === 'ar' ? 'text-right' : ''}`}>
          {value}
        </Text>
      </View>
    </View>
